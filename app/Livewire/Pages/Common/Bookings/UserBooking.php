   ...
   public function render()
{
    ...
    public function tutorAccept($bookingId)
    {
        try {
            $booking = SlotBooking::findOrFail($bookingId);

            if (Auth::id() != $booking->tutor_id) {
                $this->dispatch('showAlertMessage', type: 'error', message: 'No tienes permiso para aceptar esta tutoría.');
                return;
            }

            if ($booking->getRawOriginal('status') != 2) {
                $this->dispatch('showAlertMessage', type: 'error', message: 'Esta tutoría no está pendiente de aceptación.');
                return;
            }
            ...
            $this->dispatch('toggleModel', id: 'session-detail', action: 'hide');
            $this->dispatch('showAlertMessage', type: 'success', message: 'Has aceptado la tutoría. Esperando pago del estudiante.');

        } catch (\Exception $e) {
            Log::error('Livewire tutorAccept error: ' . $e->getMessage());
            $this->dispatch('showAlertMessage', type: 'error', message: 'Error al procesar la aceptación de la tutoría.');
        }
    }

    public function tutorReject($bookingId)
    {
        try {
            $booking = SlotBooking::findOrFail($bookingId);

            if (Auth::id() != $booking->tutor_id) {
                $this->dispatch('showAlertMessage', type: 'error', message: 'No tienes permiso para rechazar esta tutoría.');
                return;
            }

            if ($booking->getRawOriginal('status') != 2 && $booking->getRawOriginal('status') != 7) {
                $this->dispatch('showAlertMessage', type: 'error', message: 'Esta tutoría no se puede rechazar en este estado.');
                return;
            }
            ...
            $this->dispatch('toggleModel', id: 'session-detail', action: 'hide');
            $this->dispatch('showAlertMessage', type: 'success', message: 'Has rechazado la tutoría.');

        } catch (\Exception $e) {
            Log::error('Livewire tutorReject error: ' . $e->getMessage());
            $this->dispatch('showAlertMessage', type: 'error', message: 'Error al procesar el rechazo de la tutoría.');
        }
    }
}
