    ...
    public function findTutots(Request $request)
    {
        try {
            ...
            $query = User::whereHas('roles', function ($q) {
                ...
                ->withCount([
                    'userReviews as total_reviews' => function ($q) {
                        $q->whereHas('review', function ($subQ) {
                            $subQ->where('status', 'active');
                        });
                    }
                ])
                ->withAvg([
                    'userReviews as avg_rating' => function ($q) {
                        $q->join('reviews', 'reviews.id', '=', 'user_reviews.review_id')
