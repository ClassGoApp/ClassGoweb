<div id="seccion-encuesta-wrapper">
    <div class="encuesta-card-dark">
        <div class="encuesta-top-dark">
            ...
            <span class="top-text-dark">AYÚDANOS A MEJORAR</span>
            <span class="bar-accent"></span>
        </div>
        <h2 class="titulo-dark">
            Tu opinión vale oro (y descuentos)
            <br>
    ...
    <div class="encuesta-modal-card">
        ...
        <div class="modal-body">
            ...
            <div class="step-content active" data-step="1">
                ...
                <h3 class="question-text">¿Encontraste fácilmente la materia que buscabas?</h3>
                <div class="options-grid-binary">
                    <label class="option-card">
                        <input type="radio" name="p1" value="si">
                        <div class="card-inner"><span class="emoji">👍</span> <span>Sí, fue fácil</span></div>
                    </label>
                    <label class="option-card">
                ...
                <div class="action-footer">
                    <button class="btn-next" onclick="EncuestaManager.next(2)" >Siguiente ➜</button>
                </div>
            </div>

            <div class="step-content" data-step="2">
                <div class="icon-header">⭐</div>
                <h3 class="question-text">¿Qué tanto nos recomendarías?</h3>
                
                <div class="rating-feedback-wrapper">
                    <span id="rating-feedback-text">Selecciona las estrellas</span>
                </div>
                ...
                <div class="action-footer">
                    <button class="btn-next" onclick="EncuestaManager.next(3)" >Siguiente ➜</button>
                </div>
            </div>
            ...
            <div class="step-content" data-step="3">
                ...
                <div class="action-footer">
                    <button class="btn-next btn-finish" onclick="EncuestaManager.next('final')">Siguiente</button>
                </div>
            </div>
...
<style>
    ...
    .encuesta-overlay-fixed {
        ...
        background: rgba(15, 30, 45, 0.7); backdrop-filter: blur(5px);
        z-index: 2147483647; 
        display: flex; align-items: center; justify-content: center;
        opacity: 0; visibility: hidden; transition: all 0.3s ease; padding: 20px;
    }
    .encuesta-overlay-fixed.active { opacity: 1; visibility: visible; }
    ...
    .encuesta-overlay-fixed.active .encuesta-modal-card { transform: scale(1); }

    .btn-corner {
        position: absolute; top: 25px; 
        width: 36px; height: 36px; border-radius: 50%; border: none;
        background: #f1f3f5; color: #555; display: flex; align-items: center; justify-content: center;
        cursor: pointer; transition: 0.2s; 
        z-index: 20;
    }
    .btn-corner:hover { background: #e9ecef; color: #000; }
    .btn-back { left: 25px; }
    ...
    .progress-track {
        flex: 1; height: 6px; background: #e9ecef; border-radius: 10px; overflow: hidden;
    }
    .progress-fill {
        height: 100%; background: #46c7e0; width: 33%; transition: width 0.3s ease;
    }
    .step-text { font-size: 0.9rem; font-weight: 800; color: #185875; white-space: nowrap; }

    .modal-body { flex: 1; position: relative; display: flex; flex-direction: column; z-index: 5; margin-top: 50px; }
    .step-content {
        display: none; flex-direction: column; height: 100%; align-items: center; text-align: center;
        animation: fadeIn 0.4s ease; width: 100%; justify-content: space-between; padding-bottom: 10px;
    }
    .step-content.active { display: flex; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }

    .icon-header { font-size: 3rem; margin-bottom: 10px; }
    .question-text { font-size: 1.5rem; color: #185875; margin-bottom: 25px; font-weight: 700; line-height: 1.3; }
    .modal-watermark { position: absolute; top: 60%; left: 50%; transform: translate(-50%, -50%); width: 250px; opacity: 0.06; pointer-events: none; z-index: 0; }
    .modal-watermark img { width: 100%; }

    .options-grid-binary { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; width: 100%; margin-bottom: auto; }
    .option-card input { display: none; }
    .card-inner {
        border: 2px solid #eee; border-radius: 16px; padding: 30px 10px;
        display: flex; flex-direction: column; align-items: center; gap: 10px;
        cursor: pointer; transition: 0.2s; color: #555; font-weight: 600; background: white;
    }
    .option-card input:checked + .card-inner { border-color: #46c7e0; background: #f0fbff; color: #185875; box-shadow: 0 4px 12px rgba(70, 199, 224, 0.2); }
    .emoji { font-size: 2.2rem; }

    .rating-feedback-wrapper { 
        height: 30px; margin-bottom: 15px; font-weight: 700; font-size: 1.2rem;
        display: flex; justify-content: center; align-items: center; color: #888;
    }
    .stars-container { display: flex; justify-content: center; gap: 10px; margin-bottom: 20px; }
    .star-btn {
        background: none; border: none; cursor: pointer; padding: 0;
        width: 50px; height: 50px; color: #e0e0e0; transition: transform 0.2s, color 0.2s;
    }
    .star-btn svg { width: 100%; height: 100%; fill: currentColor; }
    .star-btn.filled { color: #ffc107; }
    .star-btn.active-scale { transform: scale(1.2); }
    ...
    .tag-bubble {
        background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 50px;
        padding: 8px 16px; font-size: 0.9rem; color: #666; cursor: pointer;
        transition: all 0.2s; font-weight: 500;
    }
    .tag-bubble:hover { background: #e2e6ea; }
    .tag-bubble.selected {
        background: #185875; color: white; border-color: #185875;
    }

    .textarea-wrapper { width: 100%; margin-bottom: auto; }
    .premium-input { width: 100%; padding: 15px; border: 2px solid #eee; border-radius: 12px; font-size: 1rem; outline: none; background: #fcfcfc; resize: none; font-family: inherit; }
    .premium-input:focus { border-color: #46c7e0; background: #fff; }

    .action-footer { width: 100%; margin-top: auto; display: flex; justify-content: center; padding-top: 10px; }
    .btn-next { background: #185875; color: white; border: none; padding: 14px 0; width: 200px; border-radius: 50px; font-weight: 700; font-size: 1rem; cursor: pointer; box-shadow: 0 5px 15px rgba(24, 88, 117, 0.2); transition: 0.2s; }
    .btn-next:hover { background: #114258; transform: translateY(-2px); }
    .btn-finish { background: #FB8500; box-shadow: 0 5px 15px rgba(255, 106, 43, 0.3); }
    .btn-finish:hover { background: #FB8500; }
    ...
    .tugo-circle { width: 110px; height: 110px; background: #e0f7fa; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 10px; }
    .tugo-circle img { width: 70px; }
    .final-title { color: #2ecc71; font-size: 2rem; font-weight: 800; margin-bottom: 5px; }
    ...
    .contact-form-premium { width: 100%; margin-top: 10px; opacity: 0; transform: translateY(20px); transition: 0.5s; }
    .contact-form-premium.visible { opacity: 1; transform: translateY(0); }
    .btn-redeem { background: #2ecc71; color: white; border: none; padding: 15px; width: 100%; border-radius: 12px; font-weight: 700; cursor: pointer; margin-top: 15px; font-size: 1.1rem; }
    
    /*css del mensaje de notificación del cupon */
    #toast-wrapper-fixed { position: fixed; top: 30px; left: 50%; transform: translateX(-50%); z-index: 2147483647; pointer-events: none; width: 90%; max-width: 400px; }
    .toast-premium { background: white; padding: 15px 25px; border-radius: 50px; box-shadow: 0 10px 40px rgba(0,0,0,0.2); display: flex; align-items: center; gap: 15px; animation: dropIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); border: 1px solid #eee; }
    ...
    @media (max-width: 600px) {
        ...
        .progress-header { width: 60%; top: 20px; }
        .btn-next { width: 100%; }
        .modal-body { margin-top: 50px; } 
        
        .tugo-circle { width: 90px; height: 90px; }
        .tugo-circle img { width: 50px; }
        .final-title { font-size: 1.6rem; }
        .final-desc { font-size: 0.95rem !important; }
    }
    /* 1. Bajamos la encuesta de "Infinito" a un número alto pero razonable */
    .encuesta-overlay-fixed {
        z-index: 10000 !important; /* Antes era 2147483647 */
    }

    /* 2. Subimos la Alerta y el Toast por encima de la encuesta */
    .swal2-container {
        z-index: 20000 !important; /* Mayor que 10000 para que gane */
    }
...
const EncuestaManager = (function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        starBtns.forEach(btn => {
            ...
            btn.addEventListener('click', () => { currentRating = val; updateStars(val, true); });
        });
    ...
    function validateStep(stepToCheck) {
        if (stepToCheck === 1) {
            ...
            if (!p1) { showToast('ℹ', 'Selecciona una opción', '#3498db'); return false; }
        }
        else if (stepToCheck === 2) {
            if (currentRating === 0) { showToast('ℹ', 'Selecciona una calificación', '#3498db'); return false; }
        }
    ...
    function resetForm() {
        ...
        starBtns.forEach(b => b.classList.remove('filled', 'active-scale'));
        const feedbackText = document.getElementById('rating-feedback-text');
        if(feedbackText) { feedbackText.textContent = "Selecciona las estrellas"; feedbackText.style.color = "#ccc"; }
        const tags = document.querySelectorAll('.tag-bubble');
        tags.forEach(t => t.classList.remove('selected'));
        const textarea = document.getElementById('comment-box');
        if(textarea) { textarea.value = ''; textarea.style.borderColor = '#eee'; }
        const phoneInput = document.querySelector('.contact-input');
        if(phoneInput) { phoneInput.value = ''; phoneInput.style.borderColor = '#eee'; }
    ...
    async function sendData(contact, btnToDisable = null) {
        ...
        if(btnToDisable) { btnToDisable.disabled = true; btnToDisable.innerText = "Enviando..."; }
        ...
        try {
            ...
            else { return { success: false, message: data.message || 'Error desconocido' }; }
        } catch (error) {
            console.error(error);
            return { success: false, message: 'Error de conexión' };
        } finally {
            if(btnToDisable) { btnToDisable.disabled = false; btnToDisable.innerText = "Finalizar"; }
        }
    }
    ...
    async function submitGuest() {
        ...
        if (input.value.length < 8) { showToast('⚠', 'Mínimo 8 dígitos', '#e74c3c'); return; }
        ...
        if(result.success) {
            ...
            if (esDuplicado) {
                Swal.fire({
                    ...
                    reverseButtons: true,
                    background: '#fff',
                    customClass: { popup: 'encuesta-swal-popup' }
                }).then((res) => {
    ...
    function checkAndOpen(e) {
        ...
        if (isAuth && !hasPhone) { 
            Swal.fire({ 
                ...
                background: '#fff', 
                customClass: { popup: 'encuesta-swal-popup', title: 'encuesta-swal-title' } 
            }).then((result) => { 
                if (result.isConfirmed) { window.location.href = profileUrl; } 
            }); 
            return; 
        }
        open();
    ...
    function open() { document.getElementById('modal-encuesta-overlay').classList.add('active'); next(1); }
    
    function updateStars(val, select=false) {
        const feedbackText = document.getElementById('rating-feedback-text');
        const starBtns = document.querySelectorAll('.star-btn');
        const getStarInfo = (v) => {
            if(v <= 2) return {t:"Nada probable 😞", c:"#e74c3c"};
            if(v === 3) return {t:"Tal vez 🤔", c:"#f1c40f"};
            return {t:"¡Sin duda! 🤩", c:"#2ecc71"};
        };
        const info = getStarInfo(val);
        if(feedbackText) { feedbackText.textContent = info.t; feedbackText.style.color = info.c; }
        starBtns.forEach(b => {
            const bVal = parseInt(b.dataset.val);
            if(bVal <= val) { b.classList.add('filled'); if(bVal === val) b.classList.add('active-scale'); else b.classList.remove('active-scale'); } else { b.classList.remove('filled', 'active-scale'); }
        });
    }
    ...
    return { next, submitGuest, submitAuth, checkAndOpen, close };
})();
</script>