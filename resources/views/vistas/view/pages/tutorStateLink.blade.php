...
@section('content')
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .content{ padding:2.5rem; text-align:center; }
    ...
    @keyframes breathe { 0%,100%{ transform:scale(1); opacity:.05;} 50%{ transform:scale(1.1); opacity:.1;} }
    @keyframes spin { from{transform:rotate(0deg);} to{transform:rotate(360deg);} }
    @keyframes scan { 0%,100%{ transform:translate(-2px,-2px);} 50%{ transform:translate(4px,4px);} }
    ...
    .action-info{ display:flex; align-items:center; gap:.75rem; text-align:left; }
    .action-icon-box{
      padding:.5rem;
    ...
    .btn-action:hover{ background-color:var(--slate-50); transform:translateX(7px); }
    ...
    .btn-mock:hover{ color:var(--primary-color); }
  </style>
    ...
    <div class="content">
      ...
      <div id="state-choosing" class="state-container fade-in">
        <div class="icon-wrapper">
          ...
          <div class="icon-bg" style="background-color: #219EBC15">
            <svg class="animate-scan" width="56" height="56" viewBox="0 0 24 24" fill="none"
                 stroke="var(--secundary-color)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="text-block">
          ...
          <p class="description">El estudiante está revisando tu perfil. Por favor, mantente en línea para recibir el pago.</p>
        </div>

        <div class="status-badge" style="color: var(--secundary-color); background-color: #219EBC10; border: 1px solid #219EBC20">
      ...
      <div id="state-paid" class="state-container hidden-state fade-in">
        <div class="icon-wrapper">
          ...
          <div class="icon-bg" style="background-color: #FB850010; border: 2px solid #FB850030">
            <svg width="56" height="56" viewBox="0 0 24 24" fill="none"
                 stroke="var(--terciary-color2)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="action-card">
          <div class="action-info">
            ...
            <div>
              <p class="action-label">ESTUDIANTE</p>
              <p class="action-status">Esperando</p>
            </div>
          </div>
      ...
      <div id="state-rejected" class="state-container hidden-state fade-in">
        ...
        <div class="text-block">
          ...
          <p class="description">El estudiante ha optado por otro tutor para esta sesión. ¡Sigue atento a nuevas solicitudes!</p>
        </div>
      ...
      <div id="state-expired" class="state-container hidden-state fade-in">
        ...
        <div class="text-block">
          ...
          <p class="description">El estudiante dejó de buscar tutor. Si vuelve a solicitar, te llegará otra invitación.</p>
        </div>
  ...
  <script>
    ...
    const initialStatus = @json($status ?? 'choosing'); // 'choosing' | 'paid' | 'rejected' | 'expired'

    const expiresAtMsRaw = @json($expires_at_ms ?? null);
    const secondsLeftRaw = @json($seconds_left ?? null);

    // ================== UI STATE ==================
    const STATES = ['choosing','paid','rejected','expired'];
    ...
    // tick (no se “atrasa” porque usa Date.now() vs expiresAtMs)
    const t = setInterval(renderCountdown, 1000);
