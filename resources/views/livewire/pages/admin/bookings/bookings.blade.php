                        ...
                        <table class="tb-table @if(setting('_general.table_responsive') == 'yes') tb-table-responsive @endif">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('QR Code') }}</th> <!-- Nueva columna -->
                                    <th>{{ __('Acciones') }}</th> <!-- Nueva columna -->
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($orders as $order)
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="{{ __('booking.transaction_id') }}">
                                            <span>{{ !empty($order?->orders?->transaction_id) ? $order?->orders?->transaction_id : '-' }}</span>
                                        </td>
                                        ...
                                        <td data-label="{{ __('booking.student_name') }}">
                                            <div class="tb-varification_userinfo">
                                                <strong class="tb-adminhead__img">
                                                    @if (!empty($order?->orderable?->student?->image) 
                                                        && Storage::disk(getStorageDisk())->exists($order?->orderable?->student?->image))
                                                        <img src="{{ resizedImage($order?->orderable?->student?->image,34,34) }}" 
                                        ...
                                        <td data-label="{{ __('booking.status') }}">
                                            <div class="am-status-tag">
                                                <em class="tk-project-tag 
                                                    {{ $order?->orders?->status == 'complete' ? 'tk-hourly-tag' : 'tk-fixed-tag' }}">
                                                    {{ $order?->orders?->status }}
                                                </em>
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="{{ __('general.actions') }}">
                                            @if ($order?->orders?->status == 'pending')
                                                <button class="btn btn-success btn-sm" 
                                                        wire:click="completeOrder({{ $order?->order_id }})">
                                                    {{ __('Completar') }}
                                                </button>
                                            @else
                                        ...
                                        <td data-label="Acciones">
                                            ...
                                            @if($order?->qr_image)
                                                <a href="{{ route('pay-qr', ['orderId' => $order?->order_id ]) }}" 
                                                   class="btn btn-primary btn-sm">
                                                    {{ __('Proceder a Pagar') }}
                                                </a>
                                            @else
                                                <span class="text-muted">No QR Disponible</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
