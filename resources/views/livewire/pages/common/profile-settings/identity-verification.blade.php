                    ...
                    @else
                        <fieldset>
                            ...
                            <div class="form-group">
                                <x-input-label style="color: black" for="name" class="am-important"
                                    :value="__('profile.date_of_birth')" />
                                <div class="form-group-two-wrap">
                                    <div @class([
                                        'form-control_wrap',
                                        'am-invalid' => $errors->has('form.dateOfBirth'),
                                    ])>
                                        <x-text-input class="flat-date" id="dof" data-format="F-d-Y"
                                            wire:model="form.dateOfBirth"
                                            placeholder="{{ __('profile.date_of_birth') }}" type="text"
                            ...
                            {{-- Campo: Foto personal --}}
                            <div class="form-group">
                                <x-input-label style="color: black" class="am-important" for="profile_photo"
                                    :value="__('profile.personal_photo')" />
                                <div class="am-uploadoption" x-data="{ isUploading: false }"
                                    ...
                                    <div class="tk-draganddrop"
                                        ...
                                        <label style="color: black" for="at_upload_photo" class="am-uploadfile">
                                            ...
                                            <span>{{ __('general.drop_file_here_or') }}
                                                <i>{{ __('general.click_here_file') }}</i> {{ __('general.to_upload') }}
                                                <em>{{ $fileExt }} (max. {{ $allowImageSize }} MB)</em></span>
                                        </label>
                                    </div>
                                    ...
                                    @if (!empty($form->image))
                                        <div class="am-uploadedfile">
                                            ...
                                            @else
                                                <img
                                                    src="{{ setting('_general.default_avatar_for_user') ? url(Storage::url(setting('_general.default_avatar_for_user')[0]['path'])) : url(Storage::url($form?->image)) }}" />
                                            @endif
                                            @if (method_exists($form->image, 'temporaryUrl'))
                                                <span>{{ basename(parse_url($form?->image->temporaryUrl(), PHP_URL_PATH)) }}</span>
                                            @endif
                                            <a href="#" wire:click.prevent="removeMedia('personal_photo')"
                                                class="am-delitem">
                            ...
                            @if ($user->hasRole('tutor'))
                                ...
                                <div class="form-group">
                                    <x-input-label style="color: black" for="coverphoto1" class="am-important"
                                        :value="__('profile.identification_card')" />
                                    <div class="am-uploadoption" x-data="{ isUploading: false }"
                                        wire:key="uploading-identification-card-{{ time() }}">
                                        <div class="tk-draganddrop"
                                            x-bind:class="{ 'am-dragfile': isDragging, 'am-uploading': isUploading }"
                                            x-on:drop.prevent="isUploading = true; isDragging = false"
                                            wire:drop.prevent="$upload('form.identificationCard', $event.dataTransfer.files[0])">
                                            <x-text-input name="file" type="file"
                                                id="at_upload_identification_card" x-ref="file_upload"
                                                accept="{{ !empty($allowImgFileExt)? join(',',array_map(function ($ex) {return '.' . $ex;}, $allowImgFileExt)): '*' }}"
                                            ...
                                            <label for="at_upload_identification_card" class="am-uploadfile">
                                                ...
                                                <span>{{ __('general.drop_file_here_or') }} <i>
                                                        {{ __('general.click_here_file') }} </i>
                                        ...
                                        @if (!empty($form->identificationCard))
                                            <div class="am-uploadedfile">
                                                ...
                                                @endif
                                                <a href="#" wire:click.prevent="removeMedia('identificationCard')"
                                                    class="am-delitem">
                                                    <i class="am-icon-trash-02"></i>
                                                </a>
                                            </div>
                            ...
                            <div class="form-group am-addressform">
                                <x-input-label style="color: black" for="address" class="am-important"
                                    ...
                                    @else
                                        ...
                                        @if (!empty($countries))
                                            <div class="form-group-half">
                                                <x-input-label style="color: black !important" for="country"
                                                    :value="__('profile.country')" />
                                                <div @class([
                                                    'form-control_wrap',
                                                    ...
                                                    <span class="am-select">
                                                        <select id="user_country" wire:model.live="form.country">
                                                            <option value="">
                                                                {{ __('profile.select_a_country') }}</option>
                                        ...
                                        @if (!empty($form->country) && !empty($states) && count($states) > 0)
                                            <div class="form-group-half">
                                                <x-input-label style="color: black !important" for="state"
                                                    :value="__('profile.state')" />
                                                <div @class([
                                                    'form-control_wrap',
                                                    'am-invalid' => $errors->has('form.state'),
                                                ])>
                                                    <span class="am-select">
                                                        <select class="am-select2" id="country_state"
                                                            wire:model="form.state">
                                                            <option value="">{{ __('profile.select_a_state') }}
                                                            </option>
