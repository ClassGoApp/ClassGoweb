				...
				<div class="am-searchdetail">
					<div class="am-search-userdetail">
						<div class="am-tutordetail_head">
							<div class="am-tutordetail_user">
								<figure class="am-tutorvone_img">
									...
									<img src="{{ asset('storage/' . $tutor?->profile?->image) }}" alt="{{ $tutor?->profile?->full_name }}" /> @else
									<img src="{{ setting('_general.default_avatar_for_user') ? url(Storage::url(setting('_general.default_avatar_for_user')[0]['path'])) : resizedImage('placeholder.png', 80, 80) }}" alt="{{ $tutor->profile?->full_name }}" /> @endif
								...
								<div class="am-tutordetail_user_name">
									<h3>
                                        ...
                                        @endif
                                    </h3> @if(!empty($tutor?->profile?->tagline))
									<span>{{ $tutor?->profile?->tagline }}</span> @endif
								</div>
							</div>
							...
							<ul class="am-tutorskills-list">
								...
								<li>
									...
									<ul x-data="{ open: false }">
										...
										@foreach ($tutor?->subjects as $index => $sub) @if ($index
										< 2) <li><span>{{ $sub->name }}</span></li>
								@else
								<li x-show="open"><span>{{ $sub->name }}</span></li>
								@endif @endforeach @if ($tutor?->subjects->count() > 2)
								<li>
									...
									<div class="wa-tags-list">
										...
										<div class="am-more am-custom-tooltip">
											<span class="am-tooltip-text">
                                                ...
                                                <span>{{ ucfirst( $lan->name )}}</span> @endforeach @endif
											</span>
									...
									<div class="am-tutordetail_user_badge am-user_badge">
										...
										<div class="am-custom-tooltip">
											...
											<figure class="am-shimmer">
												@if (!empty($badge?->image) && Storage::disk(getStorageDisk())->exists($badge?->image))
												<img src="{{ resizedImage($badge?->image,80,80) }}" alt="{{$badge?->name}}" /> @else
												<img src="{{ setting('_general.default_avatar_for_user') ? url(Storage::url(setting('_general.default_avatar_for_user')[0]['path'])) : resizedImage('placeholder.png', 80, 80) }}" alt="{{ $tutor->profile?->full_name }}" /> @endif
											</figure>
										</div>
										@endforeach
						...
						<div class="am-tutordetail-btns">
							<livewire:pages.tutor.action.action :tutor="$tutor" :isFavourite="$isFavourite" :navigate="false" :key="$tutor->id" />
						</div>
						@endif
...
<div class="am-tutor-detail">
		...
		<div class="am-howtobook">
			...
			<div class="am-howtobook_popup">
				...
				<figure class="am-howtobook_img">
					<img src="{{ url(Storage::url(setting('_lernen.tips_for_booking_image')[0]['path'])) }}" alt="img description">
				...
				<div class="am-howtobook_content">
					<div class="am-howtobook_info">
						...
						<h3>{{ setting('_lernen.tips_for_booking_heading') }}</h3> @endif @if (!empty(setting('_lernen.tips_for_booking_bullets')))
						<ol>
					...
					<div class="am-howtobook_info">
						...
						<h3>{{ setting('_lernen.tips_for_booking_sub_heading') }}</h3> @endif @if (!empty(setting('_lernen.tips_for_booking_sub_bullets')))
						<ol>
	...
	<div wire:ignore class="am-booking_section am-featured-courses" id="courses">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<h1>{{ __('courses::courses.featured_courses') }}</h1> @if($courses->isNotEmpty())
						...
						<div class="splide__track">
							<ul class="splide__list">
								...
								<li class="splide__slide">
									<div class="cr-card">
										<figure class="cr-image-wrapper" x-data="{ 
                                                            isOpen: false, 
											...
											<template x-if="!isOpen">
                                                        ...
                                                        </template> @if(!empty($course?->promotionalVideo?->path) && Storage::disk('public')->exists($course?->promotionalVideo?->path) )
											<template x-if="!isOpen">
                                                                <figcaption>
										...
										<div class="cr-course-card">
											...
											<div class="cr-price-container">
												<div class="cr-price-info">
													@if (!empty($course->pricing?->price) && !empty($course->pricing?->final_price) && $course->pricing?->price != 0.00 ) @if ($course->pricing?->price != $course->pricing?->final_price)
												...
												<div class="cr-duration-info">
													<i class="am-icon-time"></i> @if($course->content_length > 0)
													<span>
					...
					@else @include('livewire.components.no-record') @endif
				</div>
			</div>
		</div>
	</div>
...
@endsection @push('styles') @vite([ 'public/css/flatpicker.css', 'public/css/videojs.css', 'public/css/flags.css' ]) @if(\Nwidart\Modules\Facades\Module::has('courses') && \Nwidart\Modules\Facades\Module::isEnabled('courses'))
<link rel="stylesheet" href="{{ asset('modules/courses/css/main.css') }}"> @endif @endpush @push('scripts')
